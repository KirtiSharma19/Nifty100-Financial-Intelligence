...
def show():
    ...
    company_df = data[data["company_name"] == company]
    ...
    c1.metric("Cash From Operations", f"{latest['cash_from_operations_cr']:,.2f}")

    c2.metric("CAPEX", f"{latest['capex_cr']:,.2f}")

    c3.metric("Free Cash Flow", f"{latest['free_cash_flow_cr']:,.2f}")
    ...
    chart = {
        "Metric": ["Cash From Operations", "CAPEX", "Free Cash Flow"],
        "Value": [
    ...
    fig = px.bar(chart, x="Metric", y="Value", text="Value")
    ...
    top = data.sort_values("free_cash_flow_cr", ascending=False).head(10)
