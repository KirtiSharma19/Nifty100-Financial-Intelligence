...
class ScreenerEngine:
    ...
    def __init__(self):
        ...
        # Latest year only
        self.df["year"] = (
            self.df["year"].astype(str).str.extract(r"(\d{4})")[0].astype(int)
        )

        latest_year = self.df["year"].max()

        self.df = self.df[self.df["year"] == latest_year].copy()
    ...
    def export(self, df, filename="exports/screener_output.csv"):
    ...
    def quality_compounder(self):
        ...
        return self.screen(roe_min=15, debt_max=1, free_cash_flow_min=0, quality_min=80)

    def value_pick(self):

        return self.screen(pe_max=20, pb_max=3, debt_max=2, dividend_min=1)

    def growth_accelerator(self):

        return self.screen(revenue_cagr_min=15, pat_cagr_min=20, debt_max=2)

    def dividend_champion(self):

        return self.screen(dividend_min=2, free_cash_flow_min=0)

    def debt_free_bluechip(self):

        return self.screen(debt_max=0, roe_min=12, market_cap_min=5000)

    def turnaround_watch(self):

        return self.screen(revenue_cagr_min=10, free_cash_flow_min=0)
