...
df["cfo_quality_score"] = np.where(df["operating_activity"] > 0, 100, 40)

df["cfo_quality_label"] = np.where(
    df["operating_activity"] > 0, "High Quality", "Low Quality"
)

df["capex_intensity_pct"] = (
    abs(df["investing_activity"]) / df["operating_activity"].replace(0, np.nan)
) * 100

df["capex_label"] = np.select(
    [df["capex_intensity_pct"] < 3, df["capex_intensity_pct"].between(3, 8)],
    ["Asset Light", "Moderate"],
    default="Capital Intensive",
)

df["distress_flag"] = np.where(
    (df["operating_activity"] < 0) & (df["financing_activity"] > 0), "YES", "NO"
)

df["deleveraging_flag"] = np.where(df["financing_activity"] < 0, "YES", "NO")
...
df["capital_allocation_label"] = np.select(
    ...
    ["Growth Investment", "Debt Reduction", "Distress"],
    default="Stable",
)
...
alerts.to_csv("output/distress_alerts.csv", index=False)
