...
class PeerEngine:
    ...
    def __init__(self):
        ...
        self.df = self.df[self.df["year"] == latest].copy()
        ...
        peer = peer.rename(columns={"peer_group_name": "peer_group"})

        self.df = self.df.merge(
            peer[["company_id", "peer_group"]], on="company_id", how="left"
        )
    ...
    def calculate_percentiles(self):
        ...
        for metric in metrics:
            ...
            if higher_is_better[metric]:
                ...
                self.df[metric + "_percentile"] = (
                    self.df.groupby("peer_group")[metric].rank(pct=True, ascending=True)
                    * 100
                )
                ...
                self.df[metric + "_percentile"] = (
                    self.df.groupby("peer_group")[metric].rank(
    ...
    def export(self):
        ...
        self.df[cols].to_csv("exports/peer_comparison.csv", index=False)
