...
def create_cluster_profiles(df):
    ...
    profiles = df.groupby("cluster_id")[FEATURES].agg(["mean", "median"]).round(2)
    ...
    profiles.insert(0, "cluster_name", profiles.index.map(CLUSTER_NAMES))
...
def create_cluster_labels(df):
    ...
    labels = df[["company_id", "company_name", "cluster_id"]].copy()
    ...
    print("Missing cluster IDs:", labels["cluster_id"].isna().sum())
...
def create_correlation_heatmap(df):
    ...
    available = [col for col in KPI_CANDIDATES if col in df.columns]
    ...
    plt.title("Pearson Correlation Matrix - Financial KPIs")

    plt.tight_layout()

    plt.savefig("reports/correlation_heatmap.png", dpi=200, bbox_inches="tight")
...
def create_outlier_report(df):
    ...
    print("\nOUTLIER REPORT")
    print("=" * 70)
    print("Outliers found:", len(outlier_df))
